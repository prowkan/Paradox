...
void RenderSystem::InitSystem()
{
	...
	DescriptorHeapDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAGS::D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
	DescriptorHeapDesc.NodeMask = 0;
	DescriptorHeapDesc.NumDescriptors = 51;
	...
	HANDLE FullScreenQuadVertexShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/FullScreenQuad.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
...

	{
		...
		SAFE_DX(Device->CreateCommittedResource(&HeapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &ResourceDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_DEPTH_READ | D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, &ClearValue, UUIDOF(ResolvedDepthBufferTexture)));
...

	{
		...
		HANDLE OcclusionBufferPixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/OcclusionBuffer.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
...

	{
		...
		SAFE_DX(Device->CreateCommittedResource(&HeapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &ResourceDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, nullptr, UUIDOF(ShadowMaskTexture)));
		...
		ShadowMaskTextureUAV.ptr = CBSRUADescriptorHeap->GetCPUDescriptorHandleForHeapStart().ptr + CBSRUADescriptorsCount * Device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE::D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
		CBSRUADescriptorsCount++;

		Device->CreateUnorderedAccessView(ShadowMaskTexture, nullptr, &UAVDesc, ShadowMaskTextureUAV);
		...
		HANDLE ShadowResolvePixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/ShadowResolve.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
...

	{
		...
		HANDLE DeferredLightingPixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/DeferredLighting.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
...

	{
		...
		HANDLE SkyVertexShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/SkyVertexShader.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		HANDLE SkyPixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/SkyPixelShader.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		HANDLE SunVertexShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/SunVertexShader.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		HANDLE SunPixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/SunPixelShader.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		HANDLE FogPixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/Fog.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
...

	{
		...
		HANDLE LuminanceCalcComputeShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/LuminanceCalc.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		HANDLE LuminanceSumComputeShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/LuminanceSum.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		HANDLE LuminanceAvgComputeShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/LuminanceAvg.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
...

	{
		...
		HANDLE BrightPassPixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/BrightPass.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		HANDLE ImageResamplePixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/ImageResample.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		HANDLE HorizontalBlurPixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/HorizontalBlur.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		...
		HANDLE VerticalBlurPixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/VerticalBlur.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
...

	{
		...
		HANDLE HDRToneMappingPixelShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/HDRToneMapping.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
...
void RenderSystem::PrepareRayTracingData()
{
	for (int i = 0; i < 4000; i++)
	{
		char StaticMeshResourceName[255];

		sprintf(StaticMeshResourceName, "Cube_%d", i);

		StaticMeshResource *StaticMesh = Engine::GetEngine().GetResourceManager().GetResource<StaticMeshResource>(StaticMeshResourceName);

		RenderMesh *renderMesh = StaticMesh->GetRenderMesh();

		D3D12_RAYTRACING_GEOMETRY_DESC RayTracingGeometryDesc;
		RayTracingGeometryDesc.Flags = D3D12_RAYTRACING_GEOMETRY_FLAGS::D3D12_RAYTRACING_GEOMETRY_FLAG_NONE;
		RayTracingGeometryDesc.Triangles.IndexBuffer = renderMesh->IndexBufferAddress;
		RayTracingGeometryDesc.Triangles.IndexCount = 6 * 6 * 8 * 8;
		RayTracingGeometryDesc.Triangles.IndexFormat = DXGI_FORMAT::DXGI_FORMAT_R16_UINT;
		RayTracingGeometryDesc.Triangles.Transform3x4 = 0;
		RayTracingGeometryDesc.Triangles.VertexBuffer.StartAddress = renderMesh->VertexBufferAddress;
		RayTracingGeometryDesc.Triangles.VertexBuffer.StrideInBytes = sizeof(Vertex);
		RayTracingGeometryDesc.Triangles.VertexCount = 9 * 9 * 6;
		RayTracingGeometryDesc.Triangles.VertexFormat = DXGI_FORMAT::DXGI_FORMAT_R32G32B32_FLOAT;
		RayTracingGeometryDesc.Type = D3D12_RAYTRACING_GEOMETRY_TYPE::D3D12_RAYTRACING_GEOMETRY_TYPE_TRIANGLES;

		D3D12_BUILD_RAYTRACING_ACCELERATION_STRUCTURE_DESC BuildRayTracingAccelerationStructureDesc;
		BuildRayTracingAccelerationStructureDesc.Inputs.DescsLayout = D3D12_ELEMENTS_LAYOUT::D3D12_ELEMENTS_LAYOUT_ARRAY;
		BuildRayTracingAccelerationStructureDesc.Inputs.Flags = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS::D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_NONE;
		BuildRayTracingAccelerationStructureDesc.Inputs.NumDescs = 1;
		BuildRayTracingAccelerationStructureDesc.Inputs.pGeometryDescs = &RayTracingGeometryDesc;
		BuildRayTracingAccelerationStructureDesc.Inputs.Type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE::D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_BOTTOM_LEVEL;
		BuildRayTracingAccelerationStructureDesc.SourceAccelerationStructureData = 0;

		D3D12_RAYTRACING_ACCELERATION_STRUCTURE_PREBUILD_INFO RayTracingAccelerationStructurePreBuildInfo;

		Device->GetRaytracingAccelerationStructurePrebuildInfo(&BuildRayTracingAccelerationStructureDesc.Inputs, &RayTracingAccelerationStructurePreBuildInfo);
		...
		ResourceDesc.SampleDesc.Count = 1;
		ResourceDesc.SampleDesc.Quality = 0;
		ResourceDesc.Width = RayTracingAccelerationStructurePreBuildInfo.ResultDataMaxSizeInBytes;
		...
		SAFE_DX(Device->CreateCommittedResource(&HeapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &ResourceDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, nullptr, UUIDOF(BottomLevelRayTracingAccelerationStructures[i])));
	
		COMRCPtr<ID3D12Resource> ScratchBuffer;
		...
		ResourceDesc.SampleDesc.Count = 1;
		ResourceDesc.SampleDesc.Quality = 0;
		ResourceDesc.Width = RayTracingAccelerationStructurePreBuildInfo.ScratchDataSizeInBytes;
		...
		SAFE_DX(Device->CreateCommittedResource(&HeapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &ResourceDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr, UUIDOF(ScratchBuffer)));

		BuildRayTracingAccelerationStructureDesc.DestAccelerationStructureData = BottomLevelRayTracingAccelerationStructures[i]->GetGPUVirtualAddress();
		BuildRayTracingAccelerationStructureDesc.ScratchAccelerationStructureData = ScratchBuffer->GetGPUVirtualAddress();

		SAFE_DX(CommandAllocators[0]->Reset());
		SAFE_DX(CommandList->Reset(CommandAllocators[0], nullptr));

		CommandList->BuildRaytracingAccelerationStructure(&BuildRayTracingAccelerationStructureDesc, 0, nullptr);
...

	{
		...
		ResourceDesc.SampleDesc.Count = 1;
		ResourceDesc.SampleDesc.Quality = 0;
		ResourceDesc.Width = sizeof(D3D12_RAYTRACING_INSTANCE_DESC) * 20000;
		...
		COMRCPtr<ID3D12Resource> InstanceDescsBuffer;

		SAFE_DX(Device->CreateCommittedResource(&HeapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &ResourceDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, UUIDOF(InstanceDescsBuffer)));

		D3D12_RAYTRACING_INSTANCE_DESC *RayTracingInstanceDescs;

		SAFE_DX(InstanceDescsBuffer->Map(0, nullptr, (void**)&RayTracingInstanceDescs));

		vector<StaticMeshComponent*> StaticMeshComponents = Engine::GetEngine().GetGameFramework().GetWorld().GetRenderScene().GetStaticMeshComponents();

		for (int i = 0; i < StaticMeshComponents.size(); i++)
		{
			RayTracingInstanceDescs[i].AccelerationStructure = BottomLevelRayTracingAccelerationStructures[(i / 2) % 4000]->GetGPUVirtualAddress();
			RayTracingInstanceDescs[i].Flags = 0;
			RayTracingInstanceDescs[i].InstanceContributionToHitGroupIndex = 0;
			RayTracingInstanceDescs[i].InstanceID = 0;
			RayTracingInstanceDescs[i].InstanceMask = 0xFF;
			
			XMMATRIX TransformMatrix = StaticMeshComponents[i]->GetTransformComponent()->GetTransformMatrix();

			RayTracingInstanceDescs[i].Transform[0][0] = TransformMatrix.m[0][0];
			RayTracingInstanceDescs[i].Transform[0][1] = TransformMatrix.m[1][0];
			RayTracingInstanceDescs[i].Transform[0][2] = TransformMatrix.m[2][0];
			RayTracingInstanceDescs[i].Transform[0][3] = TransformMatrix.m[3][0];
			RayTracingInstanceDescs[i].Transform[1][0] = TransformMatrix.m[0][1];
			RayTracingInstanceDescs[i].Transform[1][1] = TransformMatrix.m[1][1];
			RayTracingInstanceDescs[i].Transform[1][2] = TransformMatrix.m[2][1];
			RayTracingInstanceDescs[i].Transform[1][3] = TransformMatrix.m[3][1];
			RayTracingInstanceDescs[i].Transform[2][0] = TransformMatrix.m[0][2];
			RayTracingInstanceDescs[i].Transform[2][1] = TransformMatrix.m[1][2];
			RayTracingInstanceDescs[i].Transform[2][2] = TransformMatrix.m[2][2];
			RayTracingInstanceDescs[i].Transform[2][3] = TransformMatrix.m[3][2];
		}
		...
		BuildRayTracingAccelerationStructureDesc.Inputs.DescsLayout = D3D12_ELEMENTS_LAYOUT::D3D12_ELEMENTS_LAYOUT_ARRAY;
		BuildRayTracingAccelerationStructureDesc.Inputs.Flags = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAGS::D3D12_RAYTRACING_ACCELERATION_STRUCTURE_BUILD_FLAG_NONE;
		BuildRayTracingAccelerationStructureDesc.Inputs.InstanceDescs = InstanceDescsBuffer->GetGPUVirtualAddress();
		BuildRayTracingAccelerationStructureDesc.Inputs.NumDescs = 20000;
		BuildRayTracingAccelerationStructureDesc.Inputs.Type = D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE::D3D12_RAYTRACING_ACCELERATION_STRUCTURE_TYPE_TOP_LEVEL;
		BuildRayTracingAccelerationStructureDesc.SourceAccelerationStructureData = 0;
		...
		SAFE_DX(Device->CreateCommittedResource(&HeapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &ResourceDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_RAYTRACING_ACCELERATION_STRUCTURE, nullptr, UUIDOF(TopLevelRayTracingAccelerationStructure)));
		...
		BuildRayTracingAccelerationStructureDesc.DestAccelerationStructureData = TopLevelRayTracingAccelerationStructure->GetGPUVirtualAddress();
		BuildRayTracingAccelerationStructureDesc.ScratchAccelerationStructureData = ScratchBuffer->GetGPUVirtualAddress();
...

	{
		...
		GlobalRootSignature.pGlobalRootSignature = ComputeRootSignature;

		D3D12_EXPORT_DESC ExportDescs[3];
		ExportDescs[0].ExportToRename = L"RayGenShader";
		ExportDescs[0].Flags = D3D12_EXPORT_FLAGS::D3D12_EXPORT_FLAG_NONE;
		ExportDescs[0].Name = L"RayGenShader";
		ExportDescs[1].ExportToRename = L"AnyHitShader";
		ExportDescs[1].Flags = D3D12_EXPORT_FLAGS::D3D12_EXPORT_FLAG_NONE;
		ExportDescs[1].Name = L"AnyHitShader";
		ExportDescs[2].ExportToRename = L"MissShader";
		ExportDescs[2].Flags = D3D12_EXPORT_FLAGS::D3D12_EXPORT_FLAG_NONE;
		ExportDescs[2].Name = L"MissShader";

		HANDLE RayTracedShadowsShaderFile = CreateFile((const wchar_t*)u"GameContent/Shaders/RayTracedShadows.dxil", GENERIC_READ, 0, NULL, OPEN_EXISTING, 0, NULL);
		LARGE_INTEGER RayTracedShadowsShaderByteCodeLength;
		BOOL Result = GetFileSizeEx(RayTracedShadowsShaderFile, &RayTracedShadowsShaderByteCodeLength);
		ScopedMemoryBlockArray<BYTE> RayTracedShadowsShaderByteCodeData = Engine::GetEngine().GetMemoryManager().GetGlobalStack().AllocateFromStack<BYTE>(RayTracedShadowsShaderByteCodeLength.QuadPart);
		Result = ReadFile(RayTracedShadowsShaderFile, RayTracedShadowsShaderByteCodeData, (DWORD)RayTracedShadowsShaderByteCodeLength.QuadPart, NULL, NULL);
		Result = CloseHandle(RayTracedShadowsShaderFile);

		D3D12_DXIL_LIBRARY_DESC DXILLibraryDesc;
		DXILLibraryDesc.DXILLibrary.BytecodeLength = RayTracedShadowsShaderByteCodeLength.QuadPart;
		DXILLibraryDesc.DXILLibrary.pShaderBytecode = RayTracedShadowsShaderByteCodeData;
		DXILLibraryDesc.NumExports = 3;
		DXILLibraryDesc.pExports = ExportDescs;

		D3D12_HIT_GROUP_DESC HitGroupDesc;
		HitGroupDesc.AnyHitShaderImport = L"AnyHitShader";
		HitGroupDesc.ClosestHitShaderImport = NULL;
		...
		RayTracingShaderConfig.MaxAttributeSizeInBytes = 2 * sizeof(float);
		RayTracingShaderConfig.MaxPayloadSizeInBytes = sizeof(float);

		D3D12_STATE_SUBOBJECT StateSubObjects[6];
		StateSubObjects[0].pDesc = &GlobalRootSignature;
		StateSubObjects[0].Type = D3D12_STATE_SUBOBJECT_TYPE::D3D12_STATE_SUBOBJECT_TYPE_GLOBAL_ROOT_SIGNATURE;
		StateSubObjects[1].pDesc = &DXILLibraryDesc;
		StateSubObjects[1].Type = D3D12_STATE_SUBOBJECT_TYPE::D3D12_STATE_SUBOBJECT_TYPE_DXIL_LIBRARY;
		StateSubObjects[2].pDesc = &HitGroupDesc;
		StateSubObjects[2].Type = D3D12_STATE_SUBOBJECT_TYPE_HIT_GROUP;
		StateSubObjects[3].pDesc = &RayTracingPipelineConfig;
		StateSubObjects[3].Type = D3D12_STATE_SUBOBJECT_TYPE::D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_PIPELINE_CONFIG;
		StateSubObjects[4].pDesc = &RayTracingShaderConfig;
		StateSubObjects[4].Type = D3D12_STATE_SUBOBJECT_TYPE::D3D12_STATE_SUBOBJECT_TYPE_RAYTRACING_SHADER_CONFIG;

		LPCWSTR Exports[3] = { L"HitGroup", L"RayGenShader", L"MissShader" };

		D3D12_SUBOBJECT_TO_EXPORTS_ASSOCIATION SubObjectToExportsAssociation;
		SubObjectToExportsAssociation.NumExports = 3;
		SubObjectToExportsAssociation.pExports = Exports;
		SubObjectToExportsAssociation.pSubobjectToAssociate = &StateSubObjects[3];

		StateSubObjects[5].pDesc = &SubObjectToExportsAssociation;
		...
		StateObjectDesc.Type = D3D12_STATE_OBJECT_TYPE::D3D12_STATE_OBJECT_TYPE_RAYTRACING_PIPELINE;

		SAFE_DX(Device->CreateStateObject(&StateObjectDesc, UUIDOF(RayTracingPipelineState)));

		COMRCPtr<ID3D12StateObjectProperties> RayTracingPipelineStateProperties;

		SAFE_DX(RayTracingPipelineState->QueryInterface<ID3D12StateObjectProperties>(&RayTracingPipelineStateProperties));

		void *HitGroupShaderIdentifier = RayTracingPipelineStateProperties->GetShaderIdentifier(L"HitGroup");
		void *RayGenShaderIdentifier = RayTracingPipelineStateProperties->GetShaderIdentifier(L"RayGenShader");
		void *MissShaderIdentifier = RayTracingPipelineStateProperties->GetShaderIdentifier(L"MissShader");
		...
		SAFE_DX(Device->CreateCommittedResource(&HeapProperties, D3D12_HEAP_FLAGS::D3D12_HEAP_FLAG_NONE, &ResourceDesc, D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, UUIDOF(ShaderBindingTableBuffer)));

		BYTE *ShaderBindingTableBufferPointer;

		SAFE_DX(ShaderBindingTableBuffer->Map(0, nullptr, (void**)&ShaderBindingTableBufferPointer));

		vector<StaticMeshComponent*> StaticMeshComponents = Engine::GetEngine().GetGameFramework().GetWorld().GetRenderScene().GetStaticMeshComponents();

		memcpy(ShaderBindingTableBufferPointer, HitGroupShaderIdentifier, D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
		memcpy(ShaderBindingTableBufferPointer + 64, RayGenShaderIdentifier, D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);
		memcpy(ShaderBindingTableBufferPointer + 128, MissShaderIdentifier, D3D12_SHADER_IDENTIFIER_SIZE_IN_BYTES);

		ShaderBindingTableBuffer->Unmap(0, nullptr);
	}
}
...
void RenderSystem::TickSystem(float DeltaTime)
{
	...
	ResourceBarriers[1].Transition.StateBefore = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_DEPTH_READ | D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
	...
	ResourceBarriers[0].Transition.StateAfter = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_DEPTH_READ | D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE;
	...
	//ResourceBarriers[0].Transition.StateAfter = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_RENDER_TARGET;
	ResourceBarriers[0].Transition.StateAfter = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
	ResourceBarriers[0].Transition.StateBefore = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
	ResourceBarriers[0].Transition.Subresource = 0;
	ResourceBarriers[0].Type = D3D12_RESOURCE_BARRIER_TYPE::D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
...

	{
		...
		D3D12_CPU_DESCRIPTOR_HANDLE SourceCPUHandles[4] = { ShadowResolveConstantBufferCBV, ResolvedDepthBufferTextureSRV, RayTracingAccelerationStructureSRV, ShadowMaskTextureUAV };
		...
		CommandList->SetPipelineState1(RayTracingPipelineState);

		/*CommandList->SetGraphicsRootDescriptorTable(3, D3D12_GPU_DESCRIPTOR_HANDLE{ ResourceGPUHandle.ptr + 0 * ResourceHandleSize });
		CommandList->SetGraphicsRootDescriptorTable(4, D3D12_GPU_DESCRIPTOR_HANDLE{ ResourceGPUHandle.ptr + 1 * ResourceHandleSize });*/
		CommandList->SetComputeRootDescriptorTable(0, D3D12_GPU_DESCRIPTOR_HANDLE{ ResourceGPUHandle.ptr + 0 * ResourceHandleSize });
		CommandList->SetComputeRootDescriptorTable(1, D3D12_GPU_DESCRIPTOR_HANDLE{ ResourceGPUHandle.ptr + 1 * ResourceHandleSize });
		CommandList->SetComputeRootDescriptorTable(3, D3D12_GPU_DESCRIPTOR_HANDLE{ ResourceGPUHandle.ptr + 3 * ResourceHandleSize });
		...
		DispatchRaysDesc.HitGroupTable.StartAddress = ShaderBindingTableBuffer->GetGPUVirtualAddress();
		DispatchRaysDesc.HitGroupTable.StrideInBytes = 32;
		DispatchRaysDesc.MissShaderTable.SizeInBytes = 32;
		DispatchRaysDesc.MissShaderTable.StartAddress = ShaderBindingTableBuffer->GetGPUVirtualAddress() + 128;
		DispatchRaysDesc.MissShaderTable.StrideInBytes = 32;
		DispatchRaysDesc.RayGenerationShaderRecord.SizeInBytes = 32;
		DispatchRaysDesc.RayGenerationShaderRecord.StartAddress = ShaderBindingTableBuffer->GetGPUVirtualAddress() + 64;
		DispatchRaysDesc.Width = ResolutionWidth;
	...
	ResourceBarriers[3].Transition.StateBefore = D3D12_RESOURCE_STATES::D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
	ResourceBarriers[3].Transition.Subresource = 0;
	ResourceBarriers[3].Type = D3D12_RESOURCE_BARRIER_TYPE::D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
