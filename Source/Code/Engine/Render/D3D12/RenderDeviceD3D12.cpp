...
void RenderDeviceD3D12::TickDevice(float DeltaTime)
{
...

	{
		...
		for (size_t k = 0; k < VisbleStaticMeshComponentsCount; k++)
		{
			...
			D3D12_CPU_DESCRIPTOR_HANDLE SourceCPUHandles[3] = { ConstantBufferCBVs[k], ((RenderTextureD3D12*)renderTexture0)->TextureSRV, ((RenderTextureD3D12*)renderTexture1)->TextureSRV };
			...
			VertexBufferViews[0].BufferLocation = ((RenderMeshD3D12*)renderMesh)->VertexBufferAddresses[0];
			VertexBufferViews[0].SizeInBytes = sizeof(XMFLOAT3) * 9 * 9 * 6;
			VertexBufferViews[0].StrideInBytes = sizeof(XMFLOAT3);
			VertexBufferViews[1].BufferLocation = ((RenderMeshD3D12*)renderMesh)->VertexBufferAddresses[1];
			VertexBufferViews[1].SizeInBytes = sizeof(XMFLOAT2) * 9 * 9 * 6;
			VertexBufferViews[1].StrideInBytes = sizeof(XMFLOAT2);
			VertexBufferViews[2].BufferLocation = ((RenderMeshD3D12*)renderMesh)->VertexBufferAddresses[2];
			VertexBufferViews[2].SizeInBytes = 3 * sizeof(XMFLOAT3) * 9 * 9 * 6;
			VertexBufferViews[2].StrideInBytes = 3 * sizeof(XMFLOAT3);

			D3D12_INDEX_BUFFER_VIEW IndexBufferView;
			IndexBufferView.BufferLocation = ((RenderMeshD3D12*)renderMesh)->IndexBufferAddress;
			...
			CommandList->SetPipelineState(((RenderMaterialD3D12*)renderMaterial)->GBufferOpaquePassPipelineState);
...

	{
		for (int i = 0; i < 4; i++)
		{
			...
			for (size_t k = 0; k < VisbleStaticMeshComponentsCount; k++)
			{
				...
				VertexBufferView.BufferLocation = ((RenderMeshD3D12*)renderMesh)->VertexBufferAddresses[0];
				...
				CommandList->SetPipelineState(((RenderMaterialD3D12*)renderMaterial)->ShadowMapPassPipelineState);
