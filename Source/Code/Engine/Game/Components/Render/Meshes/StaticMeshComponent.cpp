...
void StaticMeshComponent::LoadFromFile(LevelFile& File)
{
	...
	if (!Engine::GetEngine().GetResourceManager().IsResourceLoaded(MaterialResourceName))
	{
		...
		String Texture2Name((char*)((BYTE*)MaterialData + 4 + Texture0Name.GetLength() + 1 + Texture1Name.GetLength() + 1));
		...
		if (!Engine::GetEngine().GetResourceManager().IsResourceLoaded(Texture2Name))
		{
			struct TextureFileHeader
			{
				UINT Width;
			...
			Pointer<BYTE[]> TextureData = Pointer<BYTE[]>::Create(Engine::GetEngine().GetFileSystem().GetFileSize(Texture2Name));
			Engine::GetEngine().GetFileSystem().LoadFile(Texture2Name, TextureData);
			...
			Engine::GetEngine().GetResourceManager().AddResource<Texture2DResource>(Texture2Name, &texture2DResourceCreateInfo);
		}
		...
		materialResourceCreateInfo.Textures[2] = Engine::GetEngine().GetResourceManager().GetResource<Texture2DResource>(Texture2Name);
